...
\cls{RP} lies between \cls{P} and \cls{NP}. \cls{co-RP} is the complement of \cls{RP} (ie. the result ``Yes''/``No''/``I don't know'' is exchanged).
...
So RP and \cls{BPP} (etc) are not influenced.
...
The position of \cls{BPP} are partially unknown; partially we will discuss them
later on.
...
\textbf{Example 3.} (\probl{MAX-3SAT}) Given a \probl{3~SAT} equation $l$ with variables
...
\section{List of complexity classes}
\begin{itemize}
  \item \cls{IP}
  \item \cls{PTAS}
  \item \cls{APX}
