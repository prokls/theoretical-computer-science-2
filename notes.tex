...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Perfect matching in a bipartite graph}
  \end{center}
\end{figure}
...
For the symbolic determinant evaluation no algorithm is known (actually, the question if a specific coefficient is the coefficient of the symbolic non-zero determinant, is an \cls{NP}-complete problem).
...
%
2-SAT (polynomial evaluable with flow theory) and 3-SAT (\cls{NP}-complete). 2 or 3 literals per clause.
...
Is \cls{ZEROP} the complexity class of algebraic circuits, which evaluate a polynomial (which is identical) to a zero-polynomial.

\[
    c = c' \quad \Leftrightarrow \quad c - c' = 0
\]
This is why the name \cls{ZEROP} was chosen. A special case of this problem is exercise 1, we discussed previously.

\textbf{Problem:} Detection of \cls{ZEROP} is non-trivial (ie. is a polynomial P $\in$ ZEROP).
...
\cls{PP} (probabilistic polynomial) is not defined algorithmically.

\begin{spec}
  \given{We consider \probl{MajSat} (majority SAT). Given is a SAT equation with $n$ variables.}
  \find{Is is true that the majority of $2^n \geq 2^{n-1}+1$ assignments satisfy the equation?}
\end{spec}
...
The corresponding decision problem ($TSP_{\text{DEC}}$):
...
This problem is known as ``happy net problem''.
...
\begin{itemize}
  ...
  \item Design and analyze an interactive protocol to show that $\overline{\probl{GI}} \in \cls{IP}$ (page~\pageref{src:gi-ip}).
...
%
More than 496~complexity classes are currently maintained in the \href{https://complexityzoo.uwaterloo.ca/Complexity_Zoo}{Complexity Zoo}.
...
The following definitions got extracted from \href{https://complexityzoo.uwaterloo.ca/Complexity_Zoo}{Complexity Zoo}.
...
\begin{enumerate}
  ...
  \item Define the probabilistic complexity classes \cls{RP}, \cls{co-\cls{RP}}, \cls{BPP}, \cls{PP} and \cls{ZPP}. Define their relation towards each other and \cls{P} and \cls{NP}. What are Monte-Carlo algorithms? What are Las-Vegas algorithms?
  ...
  \item Discuss which interactive protocol variants exist and what their properties are. Discuss their relation to \cls{P} and \cls{NP}. Define the class \cls{IP}.
  ...
  \item What does the PCP-Theorem say? What does PCP stand for? When is a decision problem part of $\probl{PCP}(r(n), q(n))$?
