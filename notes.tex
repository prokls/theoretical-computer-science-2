...
\newcommand{\dateref}[1]{\textbf{Date.} #1\par}
...
\begin{itemize}
  ...
  \item Focus: Algorithms, Optimization problems, cryptography
\end{itemize}
...
\begin{enumerate}
  \item Complexity theory
    ...
    \end{itemize}
  \item approximation of complexity (complexity theory of heuristic algorithms)
\end{enumerate}
...
%
$G = (U, V, E)$ is an undirected, bipartite graph with $U = \set{u_1, u_2, \ldots, u_n}$ and $V = \set{v_1, v_2, \ldots, v_n}$.

\textbf{Question:} Does a perfect matching in the bipartite graph G exist?
...
\textbf{Proof of the Lemma:} Complete induction to $m$.
\begin{description}
  \item[$m=1$:]
...
Remarks:
\begin{itemize}
  \item $p$ results in evaluation of ``gzz'' test tuple 0.
\end{itemize}
...
If $\det A(G) (i_1, \ldots, i_m) \neq 0$ then answer ``YES, G has a perfect matching''.
If $\det A(G) (i_1, \ldots, i_m) = 0$ then answer ``I know know'' (we need further repetitions).
...
\begin{itemize}
  ...
  \item If the graph does not have a perfect matching, the answer of the algorithm is not complete.
...
\begin{itemize}
  \item In the case of matching we can see that the error probability (applying the algorithm once)
...
\textbf{Input:} A boolean equation $\phi$ in conjunctive normal form. \\
\textbf{Problem:} Is $\phi$ satisfiable?

Consider the following algorithm for SAT:
\begin{enumerate}
  \item Start with a random assignment $T$ for the variables $x_1, \ldots, x_n$.
  \item Repeat $r$ times:
    \begin{enumerate}
      \item Is satisfiable? Answer yes.
      \item $c$ is one of the clauses which are not satisfied.
            Take one literal and negate its variable in the assignment.
    \end{enumerate}
  \item Answer ``no'' or ``probably no''.
\end{enumerate}

Is also a Monte-Carlo-Algorithm.
Probability for false negatives? How to choose $r$?
...
\textbf{Given:} n is odd  \\
\textbf{Question:} Is $n$ not a prime number?
...
Given: Given an undirected graph ($G = (V, E)$) and a hamiltonian cycle $Q$ in G.
...
\[
    d_{ij} := \begin{cases}
        1 & \text{if }\set{i,j}\text{ is an edge in G or edge in G'} \\
        2 & \text{if }\set{i,j}\text{ is no edge in G but i,j are vertices of G} \\
        3 & \text{else} \\
    \end{cases}
\]
...
\textbf{Proof:}
\begin{enumerate}
  \item Using binary search and a SAT oracle we can evaluate the greatest
  ...
  \item Reduce MAX-OUTPUT to MAX-WEIGHT-SAT.
...
\text{Remark:} The question whether the given set of SAT equations $\geq K$
...
\begin{enumerate}
  \item With $i=1$, $\Sigma_1 P = \text{NP}$
  ...
  \item $i \geq 2$ and step $i - 1 \to i$
    \begin{itemize}
      \item ``$\Leftarrow$'': There exists a relation $R$ according
      ...
      \item ``$\Rightarrow$'': $L \in \Sigma_i P$ (additionally with a
...
\textbf{Given.} A SAT equation $\phi$.
...
\[
    b \in A(x) \oplus t_y \stackrel{\text{observation}}{\Leftrightarrow}
...
%
Given a graph $G = (V \cup U, E)$ with $U = \set{u_1, \ldots, u_n}$ and
$V = \set{v_1, \ldots, v_n}$. Find the number of perfect matchings in $G$
...
When does the association $\pi$ (permutation $\pi \in S_n$) describe
a perfect matching?
...
\] \[
  x \notin L \Rightarrow \forall \underbrace{P}_{\text{proof}}:
    \set{0,1}^* \rightarrow \set{0,1}^*
\] \[
  \text{with } \operatorname{propability}(\text{verifier does not
    accept }P) = 0
\]
...
%
\textbf{Given.} 2 graphs $G_0 = (V_0, E_0), G_1 = (V_1, E_1)$ with
$|V_0| = |V_1|$ and $|E_0| = |E_1|$. \\
\textbf{Question.} Are $G_0$ and $G_1$ isomorphic ($G_0 \cong G_1$)?

\subsection{Complement of graph isomorphism}
%
\textbf{Given.} (like in previous graphs) \\
\textbf{Question.} Are $G_0$ and $G_1$ not isomorph?
...
%
\textbf{Given.} Given is a room A and room B. They are separated by a magic door
...
\textbf{Question.} How can Bob verify that he knows the secret without telling
  Alice the secret?
